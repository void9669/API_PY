...
url_get = "https://playground.learnqa.ru/api/user/2"
...
class TestUserGet(BaseCase):
    ...
    def test_get_user_auth_same_user(self):
        ...
        response1 = requests.post(url_login, json=data)
        auth_sid = self.get_cookie(response1, "auth_sid")
        token = self.get_header(response1, "x-csrf-token" )
        user_id_auth = self.get_json_value(response1, "user_id")

        response2 = requests.get(f"https://playground.learnqa.ru/api/user/{user_id_auth}", 
